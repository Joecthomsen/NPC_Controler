...
/*         mbed_err =  mbedtls_aes_crypt_cbc(&cur_session->ctx_aes,
                    MBEDTLS_AES_DECRYPT,
                    PUBLIC_KEY_LEN,
                    cur_session->rand,
                    in->sc1->client_verify_data.data,
                    check_buf ); */
    ...
    mbed_err = mbedtls_aes_crypt_ctr(&cur_session->ctx_aes,
        ...
        printf("security1: Random Key 1: ");
        for (size_t i = 0; i < sizeof(cur_session->rand); i++) {
            printf("%02x", cur_session->rand[i]);
        }
        printf("\n");
    ...
    if (mbedtls_ct_memcmp(check_buf, cur_session->device_pubkey,
        ...
        // Print the values of check_buf and cur_session->device_pubkey
/*         ESP_LOGE(TAG, "check_buf: ");
        for (int i = 0; i < sizeof(cur_session->device_pubkey); i++) {
            ESP_LOGE(TAG, "%02X", check_buf[i]);
        } */
        
/*         ESP_LOGE(TAG, "device_pubkey: ");
        for (int i = 0; i < sizeof(cur_session->device_pubkey); i++) {
            ESP_LOGE(TAG, "%02X", cur_session->device_pubkey[i]);
        } */

        printf("security1: device_pubkey:");
        for (size_t i = 0; i < sizeof(cur_session->device_pubkey); i++) {
            printf("%02x", cur_session->device_pubkey[i]);
        }
        printf("\n");

        printf("security1: check_buf:");
        for (size_t i = 0; i < sizeof(cur_session->device_pubkey); i++) {
            printf("%02x", check_buf[i]);
        }
        printf("\n");

        ESP_LOGE(TAG, "sizeOf: %d", sizeof(cur_session->device_pubkey));
    ...
    printf("security1: Random Key 2 before encryption: ");
    for (size_t i = 0; i < sizeof(cur_session->rand); i++) {
        printf("%02x", cur_session->rand[i]);
    }
    printf("\n");
    ...
    printf("security1: Outbuf: ");
    for (size_t i = 0; i < sizeof(outbuf); i++) {
        printf("%02x", outbuf[i]);
    }
    printf("\n");

    printf("security1: Random Key 2 after encryption: ");
    for (size_t i = 0; i < sizeof(cur_session->rand); i++) {
        printf("%02x", cur_session->rand[i]);
    } 
    printf("\n");

    printf("security1: stb: ");
    for (size_t i = 0; i < sizeof(cur_session->stb); i++) {
        printf("%02x", cur_session->stb[i]);
    }
    printf("\n");
                                         ...
                                         const protocomm_security1_params_t *pop)
{
    ...
    if (pop != NULL && pop->data != NULL && pop->len != 0) {
        ...
        printf("security1: Key BEFORE XOR:");
        for (size_t i = 0; i < sizeof(cur_session->sym_key[i]); i++) {
            printf("%02x", cur_session->sym_key[i]);
        }
        printf("\n");
...
/*     ESP_LOGE("Your_TAG", "Shared key after XOR: ");
    for (int i = 0; i < PUBLIC_KEY_LEN; i++) {
        ESP_LOGE("Your_TAG", "%02x", cur_session->sym_key[i]);
    } */

    printf("security1: Shared key after XOR:");
    for (size_t i = 0; i < sizeof(cur_session->sym_key); i++) {
        printf("%02x", cur_session->sym_key[i]);
    }
    printf("\n");
                              ...
                              uint8_t **outbuf, ssize_t *outlen)
{
    ...
    *outbuf = (uint8_t *) malloc(*outlen);
    ...
    printf("Rand before:"); 
    for(int i = 0; i < sizeof(cur_session->rand); i++) {
    printf("%02x", cur_session->rand[i]);
    }
    printf("\n");

    printf("Sym key before:");
    for(int i = 0; i < sizeof(cur_session->sym_key); i++) {
    printf("%02x", cur_session->sym_key[i]); 
    }
    printf("\n");

    printf("Output data before:");
    for(int i = 0; i < inlen; i++) { 
    printf("%02x", (*outbuf)[i]);
    }
    printf("\n");

    printf("nc_off before: %d\n", cur_session->nc_off);

    printf("inlen: %d\n", inlen);

    // Print stb before encryption
    printf("stb before: ");
    for(int i=0; i<sizeof(cur_session->stb); i++) {
    printf("%02x", cur_session->stb[i]);
    }
    printf("\n");
 ...
 */
    ...
    //uint8_t *test_buf = (uint8_t *) malloc(inlen);
    //memset(cur_session->stb, 0, sizeof(cur_session->stb));
...
/*     mbed_err = mbedtls_aes_crypt_cbc(&cur_session->ctx_aes,
    ...
    // TEMP SHORTCUT FOR TESTING PURPOSES
    memcpy(*outbuf, inbuf, inlen);

    // Print stb after encryption
    printf("stb after: ");
    for(int i=0; i<sizeof(cur_session->stb); i++) {
        printf("%02x", cur_session->stb[i]); 
    }
...
/*     int ret2 = mbedtls_aes_crypt_ctr(&cur_session->ctx_aes, inlen, &cur_session->nc_off, 
                                cur_session->rand, cur_session->stb, inbuf, test_buf); */

    printf("mbedtls_aes_crypt_ctr status returned %d\n", ret);

    printf("nc_off after: %d\n", cur_session->nc_off);
    printf("Output data:");
    for(int i = 0; i < inlen; i++) { 
    printf("%02x", (*outbuf)[i]);
    }

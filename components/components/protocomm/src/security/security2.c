        ...
        const protocomm_security2_params_t *sv)
{
    ...
    if (in->sc0->client_pubkey.len != PUBLIC_KEY_LEN) {
        ...
        ESP_LOGI(TAG, "Received public key length: %d", in->sc0->client_pubkey.len);
    ...
    //ESP_LOGI(TAG, "Username: %.*s", in->sc0->client_username.len, in->sc0->client_username.data);
    ESP_LOGI(TAG, "Username: ");
    for (int i = 0; i < in->sc0->client_username.len ; i++) {
        ESP_LOGE(TAG, "%02X", in->sc0->client_username.data[i]);
    }    
    ...
    ESP_LOGI(TAG, "Verifyer:");
    for (size_t i = 0; i < cur_session->verifier_len; i++) {
        printf("%02x", cur_session->verifier[i]);
    }
    printf("\n");
    ...
    //ESP_LOGI(TAG, "Public key: %.*s", device_pubkey_len, device_pubkey);
    ESP_LOGI(TAG, "device_pubkey: ");
    for (int i = 0; i < device_pubkey_len; i++) {
        ESP_LOGE(TAG, "%02X", device_pubkey[i]);
    }

    ESP_LOGI(TAG, "Shared Key (Hexadecimal):");
    for (size_t i = 0; i < cur_session->session_key_len; i++) {
        printf("%02x", cur_session->session_key[i]);
    }
    printf("\n");
    ...
    ESP_LOGI(TAG, "Session key: %.*s", cur_session->session_key_len, cur_session->session_key);
    ...
    ESP_LOGI(TAG, "Stored username: %s", cur_session->username);
        ...
        SessionData *req, SessionData *resp)
{
    ...
    ESP_LOGI(TAG, "The client proof length: %d", in->sc1->client_proof.len);

    // Print the client proof in hexadecimal
    ESP_LOGI(TAG, "The client proof in hexadecimal:");
    for (int i = 0; i < in->sc1->client_proof.len; i++) {
        ESP_LOGI(TAG, "%02X ", in->sc1->client_proof.data[i]);
    }
    ...
    ESP_LOGI(TAG, "The client proof length: %d", in->sc1->client_proof.len);
    ESP_LOGI(TAG, "The client proof: %.*s", in->sc1->client_proof.len, in->sc1->client_proof.data);
                                  ...
                                  void *priv_data)
{
    ...
    if (!req) {
        ESP_LOGE(TAG, "Income Data:");
        for (size_t i = 0; i < inlen; i++) {
            ESP_LOGE(TAG, "Byte %zu: %02X", i, inbuf[i]);
        }

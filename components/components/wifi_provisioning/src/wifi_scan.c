...
static esp_err_t cmd_scan_start_handler(WiFiScanPayload *req,
                                        WiFiScanPayload *resp, void *priv_data)
{
    printf("wifi_scan.c: Scanning endpoint called...\n");
    ...
    printf("WiFiScanPayload:\n");
    printf("status: %d\n", resp->status); 
    
    ESP_LOG_BUFFER_HEX(TAG, resp, wi_fi_scan_payload__get_packed_size(resp));

    printf("wifi_scan.c: Scan started successfully\n");
    return ESP_OK;
}
                                 ...
                                 uint8_t **outbuf, ssize_t *outlen, void *priv_data)
{
    ...
    ESP_LOGI(TAG, "Response msg: %d", resp.msg);
    ...
    *outbuf = (uint8_t *) malloc(*outlen);
    ...
    ESP_LOGI(TAG, "outbuf=%p outlen=%d", *outbuf, *outlen); 
